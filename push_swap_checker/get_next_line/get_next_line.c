...
/*   Updated: 2024/01/27 11:01:45 by mafaisal         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*remaining_of_line(char *line)
{
	...
	remaining_len = ft_lentochar(line, '\0') - (ft_lentochar(line, '\n'));
	if (remaining_len == 0)
		return (free(line), NULL);
	remaining = malloc (remaining_len + 1);
	if (!remaining)
		return (NULL);
	ft_copy(remaining, line + ft_lentochar(line, '\n'), remaining_len + 1);
	free(line);
	return (remaining);
}
